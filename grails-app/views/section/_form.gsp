...
<div class="fieldcontain ${hasErrors(bean: sectionInstance, field: 'students', 'error')} ">
	...
	<g:select name="students" from="${school.User.list()}" multiple="multiple" optionKey="id" size="5" value="${sectionInstance?.students*.id}" class="many-to-many"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: sectionInstance, field: 'teacher', 'error')} required">
	...
	<g:select id="teacher" name="teacher.id" from="${school.User.list()}" optionKey="id" required="" value="${sectionInstance?.teacher?.id}" class="many-to-one"/>
</div>
